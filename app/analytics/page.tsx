...
export default function Analytics(){
 ...
 useEffect(()=>{load()},[]);
 if(loading&&!data) return <main><section className="hero"><div className="container hero-inner"><Link href="/" className="back-link">← Kembali ke klasemen</Link><div className="profile-title"><div className="eyebrow">ERA SUPER LEAGUE • V6.2</div><h1>League Analytics</h1></div></div></section><div className="container page-shell"><div className="analytics-loading card flex items-center justify-center gap-3"><RefreshCw className="w-5 h-5 animate-spin text-blue-400" /><span className="animate-pulse">Memuat data dan menghitung analytics…</span></div></div></main>;
 return <main><section className="hero"><div className="container hero-inner"><div className="hero-top"><div className="brand-pill"><Trophy size={15}/> ERA SUPER LEAGUE</div><div className="id-pill">LEAGUE ID <b>134820</b></div></div><div className="profile-title"><div className="eyebrow">2026 / 27 • ANALYTICS • V6.2</div><h1>League <span>Analytics</span></h1><p>Dashboard performa komprehensif Era Super League.</p></div><div className="hero-meta"><span><i/> FPL Data</span><span>GW {data?.current??'—'}</span><span>{fmt(data?.totalManagers??0)} manager</span>{data?.lastUpdated&&<span className="opacity-70">Updated: {new Date(data.lastUpdated).toLocaleTimeString('id-ID')}</span>}</div></div></section>
 ...
 <section className="card roadmap mt-6"><div><div className="section-kicker">V6.2 ANALYTICS — COMING SOON</div><h2>Insight berikutnya</h2>
 ...
 <footer>ERA SUPER LEAGUE • Analytics V6.2 • League ID 134820</footer></div></main>
}
